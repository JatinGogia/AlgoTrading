...
def financial_ratios(ticker, share_price):
    files = [f'recommendation/{ticker}_Income_Statement.csv', f'recommendation/{ticker}_Balance_Sheet.csv']
...
def income_statement_analysis(ticker):
    files = f'recommendation/{ticker}_Income_Statement.csv'
...
def cash_flow_analysis(ticker):
    files = f'recommendation/{ticker}_Cash_Flow.csv'
...
def recommend(ticker):
    share_price = []
    for i in range(0, 10):
        startDate = datetime(datetime.now().year-i, datetime.now().month, datetime.now().day-5)
        endDate = datetime(datetime.now().year-i, datetime.now().month, datetime.now().day)
        data =  pdr.get_data_yahoo(f"{ticker}.NS", start=startDate, end=endDate)
...
def recommend_stocks():
    values = {}
    values['RELIANCE'] = recommend('RELIANCE')
    values['ICICI'] = recommend('ICICI')
    values['HDFC'] = recommend('HDFC')
    values['SBI'] = recommend('SBI')
    values['TCS'] = recommend('TCS')
    values['INFOSYS'] = recommend('INFY')
    values['MAHINDRA & MAHINDRA'] = recommend('M&M')
    values['TECH MAHINDRA'] = recommend('TECHM')
    values['TATA MOTORS'] = recommend('TATAMOTORS')
    

    max_stocks = []
    for value in values.items():
        max_stocks.append(value)

    max_stocks.sort(reverse=True,key=lambda x:x[1])
    print(max_stocks)
    stock_list = []

    for val in range(0,3):
                stock_list.append(max_stocks[val][0])
