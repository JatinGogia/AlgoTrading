...
def fetch_data_company_wise(company_name):
    ...
    data =  pdr.get_data_yahoo(f"{company_name}.NS", start=startDate, end=endDate)
    return data
...
def model(X_test, df, TimeSteps, scaler):

    model = keras.models.load_model('recommendation/model.h5')
...
def predict_trend(cmpName):

    future_days = 15   # Take it from the user
    time_step, testing_rec = decide_steps(future_days)
    df = fetch_data_company_wise(cmpName)
    ...
    NextxDaysPrice, predicted_Price = model(X_test, 
                        df, time_step, scaler)

    diff = []
    for i in range(1, len(NextxDaysPrice)):
        prev_val = NextxDaysPrice[i-1]
        val = NextxDaysPrice[i]
        diff.append(prev_val - val)
    ...
    if count_pos >= (len(NextxDaysPrice) / 2):
        return "Uptrend"
    else:
        return "Downtrend"
